...
// return res.attachment(file.path);
  ...
  readExcel: function (req, res) {
    ...
    var _ = require("underscore");
    var wb = XLS.readFile('/Users/carloswalther/Documents/proyectos/psicotecno/docs/FACTURACIONTOTAL.xls');
    var historics = [];
    //sails.log(wb);
    for (var i = 1; i <= wb.Sheets.Sheet1["!range"].e.r; i++) {
      var dateArray = wb.Sheets.Sheet1["A" + i].w.split("/");
      //sails.log(dateArray);
      var newHistoric = {};

      newHistoric.registerDate = new Date(1 * ("20" + dateArray[2]), 1 * dateArray[0] - 1, dateArray[1] * 1);
      newHistoric.patientName = wb.Sheets.Sheet1["B" + i].w;
      if (!_.isUndefined(wb.Sheets.Sheet1["C" + i])) {
        newHistoric.patientRut = wb.Sheets.Sheet1["C" + i].w.replace(".", "").replace(".", "").replace("-", "");
      }
      if (!_.isUndefined(wb.Sheets.Sheet1["D" + i])) {
        newHistoric.examName = wb.Sheets.Sheet1["D" + i].w;
      }
      if (!_.isUndefined(wb.Sheets.Sheet1["E" + i])) {
        newHistoric.companyName = wb.Sheets.Sheet1["E" + i].w;
      }
